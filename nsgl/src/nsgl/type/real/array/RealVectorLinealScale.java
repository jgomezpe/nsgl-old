...
import nsgl.math.algebra.InvertableScale;
import nsgl.type.array.Array;
import nsgl.type.real.LinealScale01;
...
public class RealVectorLinealScale implements InvertableScale<double[]> {
    ...
    public RealVectorLinealScale( double[] originalMin, double[] originalMax,
        ...
        for( int i=0; i<scale.length; i++ ){
            scale[i] = new nsgl.type.real.LinealScale(originalMin[i], originalMax[i], targetMin[i], targetMax[i]);
        }
    }
