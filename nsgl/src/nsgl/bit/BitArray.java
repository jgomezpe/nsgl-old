...
import nsgl.random.integer.UniformInt;
...
public class BitArray implements Cloneable, Array<Boolean> {
  /**
 ...
 private int[] data = null;
  ...
  public BitArray(int n, boolean randomly) {
    ...
    data = new int[m];
    if (randomly){
      UniformInt g = new UniformInt(Int.HIGHEST_BIT >>> 1);
      RandBit rg = new RandBit();
      g.generate( data, 0, m );
      for (int i = 0; i < m; i++) {
  ...
  public static BitArray random(int limit) {
    int size = Int.getBitsNumber(limit);
 ...
 /**
  ...
  @Override
  public boolean set(Integer index, Boolean value) {
  ...
  @Override
  public boolean add(Integer index, Boolean v) { return add((int)index, (boolean)v); }

  @Override
  public void resize(int n){
	  if( n<=this.n) this.n = n;
	  else this.add(new BitArray(n-this.n, true));
  }
  ...
  @Override
  public boolean del(Boolean bit){
	  Integer i=find(bit);
